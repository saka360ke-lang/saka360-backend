...
router.post("/payout", async (req, res) => {
  try {
    ...
    if (!affiliate_email || !affiliate_name || !amount) {
      return res
        .status(400)
        .json({ error: "affiliate_email, affiliate_name, amount are required" });
    }

    const ref = payout_ref || `FT-${Math.floor(100000 + Math.random() * 900000)}`;

    // (Optional) store payout record
    try {
      await pool.query(
        `INSERT INTO affiliate_payouts
         (affiliate_email, affiliate_name, amount, currency, payout_method, payout_ref, period_label, created_at)
    ...
    // Send payout receipt using templates/payout.hbs
    await sendEmail(
      affiliate_email,
 ...
 * Body: { "user_email":"ally@example.com" }
 * Returns a simple affiliate link.
 */
